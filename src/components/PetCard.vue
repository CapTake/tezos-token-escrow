    ...
    <div class="divStyle">
        ...
        <template #finish>
          <div v-if="item.level === 0">
            ...
            <template #process="{ timeObj }">
                <progress :value="timeObj.leftTime / (leeway  * 1000)" style="transition: all 0.5s; height: 16px!important; margin: 0 auto 4px;" />
...
export default {
  ...
  computed: {
    ...
    mayMintSpecial () {
      return this.item.level > 5 && this.item.kind < 6
    },
    isDead () {
  ...
  methods: {
    ...
    onBurn () {
      ...
      this.burning = true
      this.$store.dispatch('burn', this.item.id)
    },
    onSpecial () {
      this.$emit('special', this.item.id)
    }
  }
}
</script>
